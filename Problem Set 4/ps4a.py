...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    # make the word lowercase to match SCRABBLE_LETTER_VALUES dict
    word = word.lower()

    # add up the points for each letter
    score = 0
    for letter in word:
        score += SCRABBLE_LETTER_VALUES[letter]

    # multiply letter score times the number of letters
    score *= len(word)

    # if all letters are used, adds 50 points
    if len(word) == n:
        score += 50
    return score
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                points_earned = getWordScore(word, calculateHandlen(hand))
